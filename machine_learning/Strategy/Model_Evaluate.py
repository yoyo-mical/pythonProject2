...
import pymysql
import datetime
from machine_learning.Strategy import DC
...
def model_eva(stock,state_dt,para_window,para_dc_window):
    ...
    model_test_date_start = (datetime.datetime.strptime(state_dt, '%Y-%m-%d')-datetime.timedelta(days=para_window)).strftime('%Y%m%d')
    ...
    if return_flag == 1:
        ...
        for i in range(len(model_test_date_seq)):
            sql_select = "select * from it a where a.stock_code = '%s' and a.state_dt >= '%s' order by a.state_dt asc limit 2" % (stock, model_test_date_seq[i])
    ...
    db.commit()
    db.close()
    print(stock + '   Precision : acc' + str(acc) + '   Recall : ' + str(recall) + '   F1 : ' + str(f1) + '   Acc_Neg : ' + str(acc_neg))
    return 1
...
if __name__ == '__main__':
    ...
    cur.execute(sql_truncate_model_ev_resu)
    ...
    stock_pool = df[0].to_list()

    cur.close()
    conn.close()
    stock_pool = ['603025.SH']
    for stock in stock_pool :
        ans = model_eva(stock,'2021-06-18',90,315)
