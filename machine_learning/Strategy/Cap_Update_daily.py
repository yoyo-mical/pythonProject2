...
def cap_update_daily(state_dt):
    ...
    for i in range(len(done_set)):
        ...
        sql = "select * from stock_all a where a.stock_code = '%s' and a.state_dt <= '%s' order by a.state_dt desc limit 1"\
              %(stock_code,state_dt)
        cursor.execute(sql)
        ...
        if len(done_temp) > 0:
            ...
            raise Exception
    sql_cap = "select capital,money_lock,money_rest from my_capital order by seq asc"
...
if __name__=='__main__':
    ...
    sql = "select state_dt from my_capital order by seq desc limit 1"
    cur.execute(sql)
    dtset = cur.fetchall()
    ...
    dt = (datetime.datetime.strptime(dtset[0][0], '%Y-%m-%d') + datetime.timedelta(days=1)).strftime('%Y%m%d')
    today = datetime.datetime.today().strftime('%Y%m%d')
    df = pro.trade_cal(exchange_id='', is_open=1, start_date=dt, end_date=today)
    date_temp = list(df.iloc[:, 1])
    date_seq = [(datetime.datetime.strptime(x, "%Y%m%d")).strftime('%Y-%m-%d') for x in date_temp]
    print(date_seq)

    for state_dt in date_temp:
        cap_update_daily(state_dt)